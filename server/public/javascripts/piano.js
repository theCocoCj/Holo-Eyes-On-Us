...
function sound(name) {
    if(!isPause) {
        let a = new Audio('/audios/' + name + '.mp3');
        a.volume = volume / 100;
        a.play().catch(() => {
        });
    }
...
function cambioBase() {
    ...
    if(base !== undefined)
        base.pause();
    base = new Audio('/audios/' + BTN.innerHTML + '.mp3');    // Create a new Audio object with the file path specified by the button's innerHTML, (equals to the name of th song)
    base.load();     // Load the audio file
    base.volume = volume / 100 / 10; // set the volume of the song
                ...
                for (let pos of poss) {
                    if (pointInPolygon(arr, pos)) {
                        ...
                        if (isPiano) {
                            ...
                            volume = parseInt(el.element.id.split('_')[1]);
                            ROT.className = el.element.id + '_';
                            setTimeout(() => esci = false, 800);
                            if(base !== undefined)
                                base.volume = volume / 100 / 10;
...
/** da fare da 65 a 83, da 166 a 173, da 268 a 281, da 290 a 312 **/
